...
if (isset($_POST["m_operation_id"]) && isset($_POST["m_sign"]))
{
   ...
   # Зачисляем средства
   $db->Query("UPDATE db_users_b SET premium_speed = premium_speed + '$serebro', to_referer = to_referer + '$to_referer', insert_sum = insert_sum + '$ik_payment_amount' WHERE id = '{$user_id}'");
   # Зачисляем бонус
   $db->Query("UPDATE db_users_b SET premium_speed = premium_speed + '$bonus' WHERE id = '{$user_id}'");
   ...
   VALUES ('$user_id','$purse','$ik_payment_amount','$serebro','$da','$da')");
								...
								// Сообщение 
								$message =   "
								<p>+$ik_payment_amount.00 RUB | В режим Обычный | $purse</p>
								";
